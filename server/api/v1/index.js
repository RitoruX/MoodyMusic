...
router.get('/get-music/:point', async(req, res) => {
    ...
    const music_genres = await MoodTag.find({ 'moodName': `${mood_scaling[final_mood_point]}` })
    random_genre = music_genres[0].tags[Math.floor(Math.random() * music_genres[0].tags.length)]
