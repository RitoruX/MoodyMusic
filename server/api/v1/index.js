...
router.get('/getmusic/:point', async(req, res) => {
    const ip_location = await axios.get(`http://api.ipstack.com/check?access_key=${IP_STACK_KEY}`);
    const weather_response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?zip=${ip_location.data.zip},${ip_location.data.country_code}&appid=${WEATHER_API_KEY}`);
    const temp = weather_response.data.main.temp - 273;
    if (temp < 28) { mood_point = -2; } else if (28 <= temp < 30) { mood_point = -1 } else if (30 <= temp < 32) { mood_point = 0 } else if (32 <= temp < 34) { mood_point = 1 } else if (temp >= 34) { mood_point = 2 }
    var final_mood_point = parseInt(req.params.point) + parseInt(mood_point);
    if (final_mood_point < 0) { final_mood_point = 0 } else if (final_mood_point > 4) { final_mood_point = 4 }
    if (moody_music.size == 0) { moody_music = await axios.get(`http://localhost:3000/api/v1/mood_music`).then(result => result.data) }
    ...
    const music_response = await axios.get(`https://api.mixcloud.com/search/?q=${random_genre}&type=cloudcast`).then(result => result.data);
    random_music = music_response.data[Math.floor(Math.random() * music_response.data.length)];
